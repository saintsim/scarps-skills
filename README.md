...
| --- | --- | --- |
| [`roadmap-item`](roadmap-item/SKILL.md) | `work on RM-25` / `/roadmap-item RM-25` | Picks up an **Open-Road** roadmap item by bare id and builds it in the code repo you're standing in — resolving the project from that repo's `.roadmap` pointer, because ids are per project and `RM-25` exists in more than one. Reads the item and its authorities, asks up front only if the item is too thin to build from, branches `rm-<id>-<slug>` off fresh `main`, implements, then **builds and runs the tests** before stopping for you to test. On your go-ahead: `/review-loop` → `/ship` → a paired **draft** Open-Road PR marking the item done, with evidence, the README row flipped and one item promoted to `next`. Say "don't wait for me to test" to run straight through. |
...
per project, with the build spec and the roadmap items. The code lives in each project's
own repo, which points back with a `.roadmap` file. `roadmap-item` is the bridge: stand in
the code repo, name the id, and it does the rest.

```
Sidebar/  work on RM-25   → resolve project via .roadmap → read item + spec + conventions
...
Ids are **per project** — `RM-25` exists in both `Sidebar/roadmap/` and `PackRight/roadmap/`
— so the project is always resolved from the repo you're in, never by searching for the
filename. The code PR merges first; both are opened as drafts together so you can merge the
pair in one sitting.

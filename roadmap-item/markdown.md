# roadmap-item — a roadmap kept as Open-Road markdown

Read from `SKILL.md` when the code repo's `.roadmap` has no `kind:` line (or says `markdown`). Section numbers here continue into `after-go-ahead.md`.
